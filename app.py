...
# --- 1. Item-Item Recommendation Function ---
def get_top_similar_books(book_title, n=10):
    if book_title not in cosine_sim_df.index:
        return []
    
    similar_scores = cosine_sim_df[book_title]
    # Return just the list of book titles
    similar_books = similar_scores.sort_values(ascending=False)[1:n+1].index.tolist()
    return similar_books

# --- 2. User-Specific Recommendation Function ---
...
# --- Reusable UI Grid Function ---
def display_books_grid(book_titles):
    for i in range(0, len(book_titles), 3):
        cols = st.columns(3)
        for j in range(3):
            if i + j < len(book_titles):
                book = book_titles[i + j]
                book_info = final_filtered_df[final_filtered_df['title'] == book].iloc[0]
                
                # Prevent quotes in variables from breaking HTML attributes
...
st.markdown("""
    ...
    .premium-divider {
        width: 35px; height: 3px; background: linear-gradient(90deg, #ff8a00, #e52e71);
        margin: 6px 0 12px 0; border-radius: 5px;
    }
    .premium-author {
        font-size: 13.5px; color: #c4c4c4; font-style: italic; margin-bottom: 6px;
        width: 100%; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
    }
    .premium-year {
        font-size: 11.5px; color: #888888; text-transform: uppercase;
        letter-spacing: 1.2px; font-weight: 600;
    }
    img {
        object-fit: contain; max-height: 300px; width: auto; display: block; margin: 0 auto;
    }
    hr {
        border: none !important; border-top: 10px solid #B2BEB5 !important;
        margin-top: 25px !important; margin-bottom: 25px !important;
        opacity: 1 !important; border-radius: 999px !important;
    }
    .book-column {
        position: relative; padding: 0; border: 2px solid #2b2b2b; border-radius: 12px;
        background-color: rgba(128, 128, 128, 0.05); box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);
        margin-top: 28px; margin-bottom: 15px; transition: transform 0.2s ease, box-shadow 0.2s ease;
        overflow: visible;
    }
    .book-column:hover {
    ...
    .recommendation-badge {
        ...
        width: 48px; height: 48px; border-radius: 50%; background: #28a745;
        color: white; border: 2px solid #2b2b2b; display: flex; align-items: center;
        justify-content: center; font-size: 18px; font-weight: bold;
        z-index: 10; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.25);
    }
    .extra-space { margin-top: 50px; }
    .recommendation-header {
        font-size: 15px; border-left: 5px solid #B2BEB5; padding-left: 12px; margin-left: 5px;
    }
    </style>
""", unsafe_allow_html=True)
...
# Creating Tabs
tab1, tab2 = st.tabs(["📚 Find Similar Books", "👤 Recommendations for You"])

# ==========================================
# TAB 1: Item-Item Recommendations
# ==========================================
with tab1:
    ...
    num_recommendations_t1 = st.number_input('Enter the number of recommendations:', min_value=1, max_value=50, value=10, key='num_t1')

    if 't1_recommendations' not in st.session_state:
        st.session_state.t1_recommendations = None

    if st.button('Recommend Books', key='btn_t1'):
        if book_title:
            similar_books = get_top_similar_books(book_title, num_recommendations_t1)
            if not similar_books:
                st.session_state.t1_recommendations = "⚠️ Book not found in the database."
            else:
                st.session_state.t1_recommendations = similar_books
                st.session_state.t1_book = book_title
                st.session_state.t1_num = num_recommendations_t1
        else:
            st.session_state.t1_recommendations = None
            st.warning("⚠️ Please select or enter a book title.")

    if st.session_state.t1_recommendations is not None:
        if isinstance(st.session_state.t1_recommendations, str):
            st.write(st.session_state.t1_recommendations)
        else:
            st.markdown(f"<div class='recommendation-header'>Top {st.session_state.t1_num} recommendations for '<strong>{st.session_state.t1_book}</strong>':</div><br>", unsafe_allow_html=True)
            
            # Use helper function to display grid
            display_books_grid(st.session_state.t1_recommendations)
...
# ==========================================
with tab2:
    st.markdown("### Personalized Picks Just for You")
    
    # We create a number input for the User ID. Defaulting to 277427 as per your testing script.
    user_id_input = st.number_input('Enter your User ID:', min_value=1, value=277427, step=1, key='user_id_input')
    num_recommendations_t2 = st.number_input('How many books do you want?', min_value=1, max_value=50, value=10, key='num_t2')

    if 't2_recommendations' not in st.session_state:
        st.session_state.t2_recommendations = None

    if st.button('Get My Recommendations', key='btn_t2'):
        # Check if user exists in the dataset
        if user_id_input not in final_filtered_df['userId'].values:
            st.session_state.t2_recommendations = "invalid_user"
        else:
            recs, history = get_user_recommendations(user_id_input, final_filtered_df, cosine_sim_df, k=num_recommendations_t2)
            st.session_state.t2_recommendations = recs
            st.session_state.t2_history = history
            st.session_state.t2_user = user_id_input
            st.session_state.t2_num = num_recommendations_t2

    # Displaying Results for Tab 2
    if st.session_state.t2_recommendations is not None:
        if st.session_state.t2_recommendations == "invalid_user":
            st.error("⚠️ User ID not found. Please try a valid User ID.")
        elif len(st.session_state.t2_recommendations) == 0:
            st.info(f"User {st.session_state.t2_user} has no interaction history or no recommendations could be mapped.")
        else:
            # Show User's context/history in an expander
            history_df = st.session_state.t2_history
            with st.expander(f"📚 View Reading History for User {st.session_state.t2_user}"):
                if len(history_df) > 0:
                    st.dataframe(history_df, hide_index=True, use_container_width=True)
                else:
                    st.write("No explicit ratings found (Only implicit interactions recorded).")

            st.markdown(f"<div class='recommendation-header'>✨ Top {st.session_state.t2_num} Recommendations for User <strong>{st.session_state.t2_user}</strong>:</div><br>", unsafe_allow_html=True)
            
            # Use helper function to display grid
            display_books_grid(st.session_state.t2_recommendations)
