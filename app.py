...
def load_and_prepare_data():
    ...
    # URL replacements for specific books
...
def get_top_similar_books(book_title, n=10):
    """Return top‑n similar books for a given title (item‑to‑item)."""
...
def get_user_recommendations(user_id, df, sim_matrix, k=10):
    """
    Generate personalised recommendations for a given user_id.
    Returns:
        list of recommended book titles,
        DataFrame of user’s top rated books (for display).
    """
    user_history_all = df[df['userId'] == user_id]['title'].tolist()
    user_history_rated = df[(df['userId'] == user_id) & (df['rating'] > 0)][
        ['title', 'rating']
    ...
    # Aggregate similarity scores from all books the user has interacted with
    scores = {}
    ...
    top_recs = [x[0] for x in sorted_scores[:k]]
    return top_recs, user_history_rated
...
""", unsafe_allow_html=True)
st.image('https://img.freepik.com/premium-vector/bookcase-with-books_182089-197.jpg',
         use_container_width=True)

# ----- Tabs -----
tab1, tab2 = st.tabs(["📚 Find Similar Books", "👤 Personalized Recommendations"])
...
with tab1:
    ...
    if st.session_state.recommendations is not None:
        ...
        if isinstance(similar_books, str):
            ...
            st.markdown(
                f"<div class='recommendation-header'>Top {rec_num} recommendations for "
                f"'<strong>{rec_book}</strong>':</div>",
                unsafe_allow_html=True
            )
            st.write("")
...
# ================== TAB 2: USER PERSONALISED ==================
with tab2:
    st.subheader("Get Personalised Recommendations")
    user_id_input = st.number_input("Enter User ID:", min_value=1, step=1, value=1, key='user_id')
    num_recs_user = st.number_input("Number of recommendations:", min_value=1, max_value=50, value=10, key='num_recs_user')

    if st.button("Get Recommendations", key='btn_tab2'):
        recs, user_history = get_user_recommendations(
            user_id_input, final_filtered_df, cosine_sim_df, k=num_recs_user
        )

        if not recs:
            st.warning("No recommendations could be generated. The user may not exist or has no interactions.")
        else:
            st.markdown(f"**Top {num_recs_user} recommendations for User {user_id_input}:**")
            if not user_history.empty:
                with st.expander("User's Reading History (Top Rated)"):
                    st.dataframe(user_history.head(10), use_container_width=True)

            # Display in the same card grid
